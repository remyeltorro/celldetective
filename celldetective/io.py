...
from celldetective.utils import ConfigSectionMap, extract_experiment_channels, _extract_labels_from_config, \
	get_zenodo_files, download_zenodo_file
import json
import threading
from skimage.measure import regionprops_table
from celldetective.utils import _estimate_scale_factor, _extract_channel_indices_from_config, _extract_channel_indices, \
	ConfigSectionMap, _extract_nbr_channels_from_config, _get_img_num_per_channel, normalize_per_channel
import matplotlib.pyplot as plt
from celldetective.filters import std_filter, median_filter, gauss_filter
from stardist import fill_label_holes
from celldetective.utils import interpolate_nan
...
def relabel_segmentation(labels, data, properties,
						 column_labels={'track': "track", 'frame': 'frame', 'y': 'y', 'x': 'x', 'label': 'class_id'},
						 threads=1):
	"""
	...
	df = pd.DataFrame(data,
					  columns=[column_labels['track'], column_labels['frame'], column_labels['y'], column_labels['x']])
...
def load_napari_data(position, prefix="Aligned", population="target", return_stack=True):
	...
	position = position.replace('\\', '/')
	if population.lower() == "target" or population.lower() == "targets":
		napari_data = np.load(position + os.sep.join(['output', 'tables', 'napari_target_trajectories.npy']),
							  allow_pickle=True)
	elif population.lower() == "effector" or population.lower() == "effectors":
		napari_data = np.load(position + os.sep.join(['output', 'tables', 'napari_effector_trajectories.npy']),
							  allow_pickle=True)
...
def control_segmentation_napari(position, prefix='Aligned', population="target", flush_memory=False):
	...
	def export_annotation():
		...
		if "Shapes" in viewer.layers:
			...
			for k, sq in enumerate(squares):
				...
				save_tiff_imagej_compatible(
					annotation_folder + f"{exp_name}_{position.split(os.sep)[-2]}_{str(t).zfill(4)}_roi_{xmin}_{xmax}_{ymin}_{ymax}_labelled.tif",
					labels_layer[xmin:xmax, ymin:ymax].astype(np.int16), axes='YX')
				save_tiff_imagej_compatible(
					annotation_folder + f"{exp_name}_{position.split(os.sep)[-2]}_{str(t).zfill(4)}_roi_{xmin}_{xmax}_{ymin}_{ymax}.tif",
					multichannel, axes='CYX')
		...
		if fov_export:
			...
			multichannel = np.array(multichannel)
			save_tiff_imagej_compatible(
				annotation_folder + f"{exp_name}_{position.split(os.sep)[-2]}_{str(t).zfill(4)}_labelled.tif",
				labels_layer, axes='YX')
			save_tiff_imagej_compatible(
				annotation_folder + f"{exp_name}_{position.split(os.sep)[-2]}_{str(t).zfill(4)}.tif", multichannel,
				axes='CYX')
			info = {"spatial_calibration": spatial_calibration, "channels": list(channel_names)}
...
def normalize(frame, percentiles=(0.0, 99.99), values=None, ignore_gray_value=0., clip=False, amplification=None,
			  dtype=float):
	"""
...
def load_frames(img_nums, stack_path, scale=None, normalize_input=True, dtype=float,
				normalize_kwargs={"percentiles": (0., 99.99)}):
	"""
