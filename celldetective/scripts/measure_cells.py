...
from celldetective.utils import extract_experiment_channels, _extract_channel_indices_from_config, \
    _extract_channel_indices, ConfigSectionMap, _extract_nbr_channels_from_config, _get_img_num_per_channel, \
...
parser.add_argument("--mode", default="target", choices=["target", "effector", "targets", "effectors"],
...
if os.path.exists(instr_path):
    ...
    if 'normalisation' in instructions:
        normalisation = instructions['normalisation']
    ...
    for t in tqdm(indices, desc="frame"):
        ...
        if do_features:
            feature_table = measure_features(img, lbl, features=features, border_dist=border_distances,
                                             ...
                                             normalisation_list=normalisation, spot_detection=spot_detection)
